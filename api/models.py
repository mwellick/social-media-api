...
class Unfollow(models.Model):
    ...
    def unique_unfollow(unfollower: str, unfollowed: str, error_to_raise):
        ...
        if Unfollow.objects.filter(
            unfollower__username=unfollower, unfollowed_user__username=unfollowed
        ).exists():
    ...
    def clean(self):
        Unfollow.unique_unfollow(
            self.unfollower.username, self.unfollowed_user.username, ValueError
        )
    ...
    def save(
        ...
        self.full_clean()
        super(Unfollow, self).save(force_insert, force_update, using, update_fields)
